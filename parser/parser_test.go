...
func TestRedirectStdoutStderrToSameFile(t *testing.T) {
	input := "grep git | wc &> outfile.txt < infile.txt"
	p := NewParser(input)
	cc := p.ParseCommand()
	...
	if !cc.MergeOutErr {
		t.Error("Should send stdout and stderr to the same file")
	}
}

func TestAppendStdoutStderrToSameFile(t *testing.T) {
	input := "grep git | wc &>> outfile.txt < infile.txt"
	p := NewParser(input)
	cc := p.ParseCommand()
